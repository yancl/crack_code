...
def get_binary_frame(fname, with_border=True, need_removedot=False, need_floodfill=False):
...
def trainmatch(region,samples):
    ...
    first=dists[0][1]
    for i in xrange(1,min(4, len(dists))):
        if dists[i][1]==first:
            confidence+=0.05
            
    if confidence<0.75:
        for i in xrange(0, min(10, len(dists))):
            print dists[i]
...
def do_min_crop(region):
    croppos = get_min_crop(region)
...
def dorotate(region):
    ...
    for i in xrange(-15,15):
        dens = density(do_min_crop(region.rotate(i)))
        if dens > maxdens:
            deg = i
            maxdens = dens
            
    #printregion( do_min_crop( region.rotate(deg) ) )
    return region.rotate(deg)

def normalize(im, rotate=True, purify=True):
    regions = imdiv(im)
    ...
    for k in xrange(len(regions)):
        if rotate:
            ...
            regions[k] = do_min_crop(regions[k])

        if purify:
            regions[k] = purify(regions[k])

    return regions

def train(im, rotate=True, purify=True, minpixelcount=20):
    global samples
    try:
    ...
    regions = normalize(im, rotate, purify)
    for region in regions:
        if pixelcount(region) <= minpixelcount:
            print 'pixelcount:', pixelcount(region)
            printregion(region)
            continue
        (w,h)=region.size
...
def crackcode(im, rotate=True, purify=True, minpixelcount=20, codelen=4):
    global samples
    if not samples:
        samples = loadsamples()
    regions = normalize(im, rotate, purify)
    ...
    ans = []
    for r in regions:
        if pixelcount(r) <= minpixelcount:
            print 'pixelcount:%d less than min:%d,pass' % (pixelcount(r), minpixelcount)
            continue
...
def divide_hs(filename):
    ...
    regions = normalize(im, rotate=False, purify=False)
    for i in xrange(len(regions)):
        regions[i].save('divs/t_%d_%s' % (i,filename), 'JPEG')
    im.save('t_%s' % filename, 'JPEG')

def train_hs(filename):
    im = get_binary_frame(filename)
    train(im, rotate=False, purify=False, minpixelcount=10)

def crackcode_hs(filename):
    im = get_binary_frame(filename)
    codes = crackcode(im, rotate=False, purify=False, minpixelcount=10, codelen=4)
    print 'codes:', codes


if __name__ == '__main__':
    if len(sys.argv) != 3:
        exit('Usage: ./xman.py {divide|train|crack} filename')
    else:
        if sys.argv[1] == 'divide':
            extract_hs(sys.argv[2])
        elif sys.argv[1] == 'train':
            train_hs(sys.argv[2])
        elif sys.argv[1] == 'crack':
            crackcode_hs(sys.argv[2])
